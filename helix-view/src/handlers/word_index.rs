...
pub(crate) fn register_hooks(handlers: &Handlers) {
  ...
  register_hook!(move |event: &mut DocumentDidClose<'_>| {
    if event.doc.word_completion_enabled() {
      helix_event::send_blocking(&tx, Event::Delete(event.doc.id(), event.doc.text().clone()));
    }
    Ok(())
  });
...
mod tests {
  ...
  fn assert_coalesces(observations: &[(&str, &str)]) {
    let change =
      coalesce(observations.iter().copied()).expect("a coalesced change should have been emitted");
