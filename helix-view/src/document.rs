...
impl Document {
  ...
  pub fn auto_pairs<'a>(
    ...
    self
      ...
      .and_then(|syntax| {
        ...
        let layer =
          syntax.layer_for_byte_range(u32::try_from(start).unwrap(), u32::try_from(end).unwrap());
  ...
  pub fn set_document_highlights(&mut self, view_id: ViewId, ranges: Vec<std::ops::Range<usize>>) {
    if ranges.is_empty() {
      self.document_highlights.remove(&view_id);
    } else {
      self
...
mod test {
  ...
  fn changeset_to_changes_ignore_line_endings() {
    ...
    let transaction = Transaction::change(doc.text(), vec![(5, 7, Some("\n".into()))].into_iter());
