...
mod dir {
    ...
    impl Ignore {
        ...
        fn add_child_path_with_found_ignore_files(
            ...
            let ig_matcher =
                if !self.inner.opts.ignore || !ignore_files_list.is_none_or(|i| i.has_ignore) {
                    Gitignore::empty()
                } else {
                    let (m, err) = create_gitignore(
                        &dir,
                        ...
                        self.parents()
                            ...
                            .map_or(path, |ig| {
                                ...
                                let without_dot_slash =
                                    strip_if_is_prefix("./", ig.inner.dir.as_path());
...
pub mod types {
    ...
    use crate::{
        ignore::Error, ignore::Match, ignore::default_types::DEFAULT_TYPES,
        ignore::pathutil::file_name,
    };
...
mod walk {
    ...
    impl DirEntryRaw {
        ...
        fn from_path(depth: usize, pb: PathBuf, link: bool) -> Result<DirEntryRaw, Error> {
            let md =
                fs::metadata(&pb).map_err(|err| Error::Io(err).with_depth(depth).with_path(&pb))?;
            Ok(DirEntryRaw {
                path: pb,
                ty: md.file_type(),
                follow_link: link,
