...
pub mod tasks {
  ...
  pub fn docgen() -> Result<(), DynError> {
    use crate::docgen::{
      LANG_SUPPORT_MD_OUTPUT, STATIC_COMMANDS_MD_OUTPUT, TYPABLE_COMMANDS_MD_OUTPUT, lang_features,
      static_commands, typable_commands, write,
    };
  ...
  pub fn indentcheck(languages: impl Iterator<Item = String>) -> Result<(), DynError> {
    use helix_core::{
      Syntax,
      indent::{IndentStyle, is_opaque_interior, is_outdent_token_at, treesitter_indent_for_pos},
    };
    ...
    for path in entries {
      ...
      for i in 0..doc.len_lines() {
          ...
          && let Some(next_pos) = text.line(i + 1).first_non_whitespace_char()
        {
          ...
          let next_byte =
            u32::try_from(text.char_to_byte(text.line_to_char(i + 1) + next_pos)).unwrap();
          ...
          if !next_is_comment {
            ...
            let leading_outdent = || {
              ...
              is_outdent_token_at(indent_query, &syntax, text, u32::try_from(byte).unwrap())
            };
