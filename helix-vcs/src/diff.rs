...
impl DiffHandle {
  ...
  fn update_document_impl(&self, text: Rope, is_base: bool, render_lock: Option<RenderLock>) -> bool {
    let event = Event {
      text,
      is_base,
      render_lock,
...
impl Diff<'_> {
  ...
  pub fn prev_hunk(&self, line: u32) -> Option<u32> {
    ...
    let res = self.diff.hunks.binary_search_by_key(&line, |hunk| hunk_range(hunk).end);
    ...
    match res {
      ...
      Ok(pos) if !hunk_range(&self.diff.hunks[pos]).is_empty() => Some(u32::try_from(pos).unwrap()),
  ...
  fn next(&mut self) -> Option<Self::Item> {
    ...
    loop {
      ...
      if (hunk_range(hunk).end as usize) < *start_line {
        ...
        self.cursor += self.hunks[self.cursor..].partition_point(|hunk| (hunk_range(hunk).end as usize) < *start_line);
