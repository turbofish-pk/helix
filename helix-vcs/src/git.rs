...
use helix_ext::git::{
    helix_discover_upwards_opts, helix_real_path, ByteSlice, Commit, EntryKind, GixDirEntryStatus,
    IndexAsWorktreeChange, IndexAsWorktreeEntryStatus, IndexWorktreeItem, ObjectId, Repository,
...
pub fn get_diff_base(file: &Path) -> Result<Vec<u8>> {
    ...
    eprintln!(
        "head: {:?}",
        head.as_ref().map(|_| "ok").map_err(|e| format!("{e:#}"))
    );
    let head = head?;
...
fn git_status(repo: &Repository, f: impl Fn(Result<FileChange>) -> bool) -> Result<()> {
    ...
    let status_platform = repo.helix_repository_status()?;
    ...
    for item in status_iter {
        ...
        let change = match index_item {
            ...
            IndexWorktreeItem::Rewrite {
                ...
                from_path: work_dir.join(source.helix_rela_path().to_os_str().map(Path::new)?),
                to_path: work_dir.join(dirwalk_entry.rela_path.to_os_str().map(Path::new)?),
            },
