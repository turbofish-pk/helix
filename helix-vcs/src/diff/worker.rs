...
use crate::diff::{ALGORITHM, DIFF_DEBOUNCE_TIME_ASYNC, DIFF_DEBOUNCE_TIME_SYNC, DiffInner, Event, RenderLock};
...
impl DiffWorker {
  ...
  fn perform_diff(&mut self, input: &InternedInput<RopeSlice>) {
    self
      .diff_alloc
      .compute_with(ALGORITHM, &input.before, &input.after, input.interner.num_tokens());
...
impl EventAccumulator {
  ...
  async fn accumulate_debounced_events(
    ...
    match self.render_lock.take() {
      ...
      Some(RenderLock { lock, timeout: None }) => {
        tokio::spawn(async move {
          diff_finished_notify.notified().await;
          drop(lock);
        });
