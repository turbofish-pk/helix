...
pub fn textobject_paragraph(
  ...
  let next_line_empty = line + 1 >= slice.len_lines() || rope_is_line_ending(slice.line(line + 1));
  let last_char =
...
mod test {
  ...
  fn test_textobject_paragraph_inside_single() {
    ...
    for (before, expected) in tests {
      ...
      let selection = selection.transform(|r| textobject_paragraph(text.slice(..), r, Inside, 1));
  ...
  fn test_textobject_paragraph_inside_double() {
    ...
    for (before, expected) in tests {
      let (s, selection) = crate::test::print(before);
      let text = Rope::from(s.as_str());
      let selection = selection.transform(|r| textobject_paragraph(text.slice(..), r, Inside, 2));
      let actual = crate::test::plain(s.as_ref(), &selection);
      assert_eq!(actual, expected, "\nbefore: `{before:?}`");
    }
  }
  ...
  fn test_textobject_paragraph_around_single() {
    ...
    for (before, expected) in tests {
      let (s, selection) = crate::test::print(before);
      let text = Rope::from(s.as_str());
      let selection = selection.transform(|r| textobject_paragraph(text.slice(..), r, Around, 1));
      let actual = crate::test::plain(s.as_ref(), &selection);
      assert_eq!(actual, expected, "\nbefore: `{before:?}`");
    }
  }
