...
fn add_indent_level(
  ...
  if added_indent_level >= 0 {
    ...
    let base_indent_width =
      crate::visual_coords_at_pos(base_indent_rope, base_indent_rope.len_chars(), tab_width).col;
    let target_indent_width = base_indent_width
      .saturating_sub((-added_indent_level).cast_unsigned() * indent_style.indent_width(tab_width));
    #[allow(deprecated)]
