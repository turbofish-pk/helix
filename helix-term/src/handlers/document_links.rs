    ...
    .filter_map(|language_server| {
      ...
      Some(async move {
        ...
        let links: Vec<_> = links
          .into_iter()
          .filter_map(|link| {
            let start = helix_lsp::util::lsp_pos_to_pos(&text, link.range.start, offset_encoding)?;
            let end = helix_lsp::util::lsp_pos_to_pos(&text, link.range.end, offset_encoding)?;
            if start > end {
              return None;
            }
            Some(DocumentLink {
