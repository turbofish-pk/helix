...
impl Application {
  pub fn new(
    ...
    let backend =
      TerminaBackend::new((&config.editor).into()).context("failed to create terminal backend")?;
    ...
    match call {
        ...
        match notification {
              ...
              .has_component(std::any::type_name::<ui::Prompt>()) =>
          {
            ...
            if self.editor.config().lsp.display_progress_messages {
              let title = title.or_else(|| self.lsp_progress.title(server_id, &token));
          ...
          Notification::Exit => {
            let status = format!("Language server exited: {}", language_server!().name());
            self.editor.set_status(status);
      ...
      Call::MethodCall(helix_lsp::jsonrpc::MethodCall {
        ...
        let reply = match MethodCall::parse(&method, params) {
          ...
          Ok(MethodCall::WorkspaceFolders) => Ok(json!(&*language_server!().workspace_folders())),
          ...
          Ok(MethodCall::ShowMessageRequest(params)) => {
            if let Some(actions) = params.actions.filter(|a| !a.is_empty()) {
              let id = id.clone();
              let select =
                ui::Select::new(params.message, actions, (), move |editor, action, event| {
                  ...
                  if let Some(language_server) = editor.language_server_by_id(server_id)
                    && let Err(err) = language_server.reply(id.clone(), Ok(json!(reply)))
                  {
                    log::error!(
