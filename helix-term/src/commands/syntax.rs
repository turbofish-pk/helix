  ...
  iter::from_fn(move || {
    loop {
      ...
      if pattern.is_some_and(|re| {
        !re.is_match(
          text.regex_input_at_bytes(name_byte_range.start as usize..name_byte_range.end as usize),
        )
      }) {
        continue;
      }
...
pub fn syntax_symbol_picker(cx: &mut Context) {
  ...
  let columns = vec![
    PickerColumn::new("kind", |tag: &Tag, ()| tag.kind.as_str().into()),
    PickerColumn::new("name", |tag: &Tag, ()| tag.name.as_str().into()),
  ];
  ...
  .with_preview(|_editor, tag| Some((tag.doc.path_or_id()?, Some((tag.start_line, tag.end_line)))))
...
pub fn syntax_workspace_symbol_picker(cx: &mut Context) {
  ...
  let get_tags =
    |query: &str, editor: &mut Editor, state: Arc<SearchState>, injector: &Injector<_, _>| {
      if query.len() < 3 {
      ...
      for doc in editor.documents() {
        ...
        let uri_or_id = doc
          .uri()
          .map_or_else(|| UriOrDocumentId::Id(doc.id()), UriOrDocumentId::Uri);
      ...
      let matcher = match state.regex_matcher_builder.build(query) {
        ...
        Err(err) => {
          log::info!("Failed to compile search pattern in workspace symbol search: {err}");
