...
use super::*;
use crate::job::Job;
use IndentStyle::{Spaces, Tabs};
use LineEnding::{Crlf, LF};
use std::fmt::Write;
      ...
      let callback = async move {
        let call: job::Callback = job::Callback::EditorCompositor(Box::new(
          move |editor: &mut Editor, compositor: &mut Compositor| {
            let picker = ui::file_picker(editor, &path.into_owned()).with_default_action(action);
...
fn reload_all(
  ...
  let docs_view_ids: Vec<(DocumentId, Vec<ViewId>)> = cx
    ...
    .documents_mut()
    .map(|doc| {
      let mut view_ids: Vec<_> = doc.selections().keys().copied().collect();
      ...
      let call: job::Callback = Callback::EditorCompositor(Box::new(
        move |_editor: &mut Editor, compositor: &mut Compositor| {
          let columns = [ui::PickerColumn::new(
            "title",
            |(_ls_id, command): &(_, helix_lsp::lsp::Command), ()| command.title.as_str().into(),
          )];
...
fn lsp_restart(
  ...
  for server in &language_servers {
      ...
      .transpose()
    {
      ...
      // in the arguments.
      Err(helix_lsp::Error::ExecutableNotFound(_)) if !args.iter().any(|arg| arg == server) => {}
    ...
    Value::String(value) => {
      ...
      Value::String(
        values
          ...
          .nth(1)
          .map_or_else(|| &values[0], AsRef::as_ref)
          .to_string(),
      )
    }
...
fn complete_register_expansion(
  ...
  fuzzy_match(content, register_names, false)
    .into_iter()
    .map(|(name, _)| (offset.., name.clone().into()))
    .collect()
}
