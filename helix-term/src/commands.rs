...
use helix_core::{
  Deletion, LineEnding, Position, Range, Rope, RopeReader, RopeSlice, Selection, SmallVec, Syntax,
  Tendril, Transaction, char_idx_at_visual_offset,
  chars::char_is_word,
...
use insert::{
  append_char_interactive, delete_char_backward, delete_char_forward, delete_word_backward,
  delete_word_forward, insert_char, insert_char_interactive, insert_newline, insert_tab, smart_tab,
};
...
macro_rules! static_commands {
    ( $($name:ident, $doc:literal,)* ) => {
        ...
        pub const STATIC_COMMAND_LIST: &'static [Self] = {
            static COMMANDS: &[MappableCommand] = &[
                $( MappableCommand::$name, )*
            ];
            COMMANDS
        };
    }
  ...
  let selection = doc.selection(view.id).clone().transform(|range| {
    ...
    let pos = match direction {
      Direction::Forward => {
        ...
        let line = cursor_line + count - 1 + usize::from(on_edge);
        if line >= text.len_lines() - 1 {
          return range;
        }
        line_end_char_index(&text, line) - usize::from(!inclusive)
      }
      Direction::Backward => {
        if inclusive {
          let line = cursor_line.cast_signed() - count.cast_signed();
          if line < 0 {
            return range;
          }
          line_end_char_index(&text, line.cast_unsigned())
        } else {
          let on_edge = text.line_to_char(cursor_line) == cursor_anchor;
          let line = cursor_line.cast_signed() - count.cast_signed() + 1 - isize::from(on_edge);
          if line <= 0 {
            return range;
          }
          text.line_to_char(line.cast_unsigned())
        }
      }
    };
        ...
        let selection = doc.selection(view.id).clone().transform(|range| {
          ...
          search::find_nth_char(count, text, ch, search_start_pos, direction)
            ...
            .map(|pos| match (inclusive, direction) {
              (true, Direction::Forward | Direction::Backward) => pos,
...
pub fn scroll(cx: &mut Context, offset: usize, direction: Direction, sync_cursor: bool) {
  ...
  let offset = match direction {
    Forward => offset.cast_signed(),
    Backward => -(offset.cast_signed()),
  };
  ...
  (view_offset.anchor, view_offset.vertical_offset) = char_idx_at_visual_offset(
    doc_text,
    view_offset.anchor,
    view_offset.vertical_offset.cast_signed() + offset,
    0,
    &text_fmt,
    // &annotations,
  ...
  match direction {
    Forward => {
      ...
      (head, off) = char_idx_at_visual_offset(
        doc_text,
        view_offset.anchor,
        (view_offset.vertical_offset + scrolloff).cast_signed(),
        0,
        &text_fmt,
        &annotations,
      );
      head += usize::from(off != 0);
      if head <= cursor {
        return;
      }
    }
    Backward => {
      head = char_idx_at_visual_offset(
        doc_text,
        view_offset.anchor,
        (view_offset.vertical_offset + height - scrolloff - 1).cast_signed(),
        0,
        &text_fmt,
        &annotations,
      )
  ...
  let picker = Picker::new(
    ...
    move |cx,
      ...
      if line_start >= text.len_lines() {
        cx.editor
          .set_error("The line you jumped to does not exist anymore because the file has changed.");
        return;
      }
...
fn file_picker_in_current_buffer_directory(cx: &mut Context) {
  ...
  let path = if let Some(path) = doc_dir {
    ...
    if !cwd.exists() {
      cx.editor
        .set_error("Current buffer has no parent and current working directory does not exist");
      return;
    }
    cx.editor
      .set_error("Current buffer has no parent, opening file picker in current working directory");
    cwd
  };
  ...
  fn stylize<'a>(&self, path: Option<&'a Path>, line: Option<usize>) -> Cell<'a> {
    ...
    if let Some(path) = path {
      let directories = path
        ...
        .unwrap_or_default();
      spans.push(Span::styled(directories, self.directory));
    }
    ...
    if let Some(line) = line {
      spans.extend([
        Span::styled(":", self.colon),
        Span::styled((line + 1).to_string(), self.number),
      ]);
    }
...
fn jumplist_picker(cx: &mut Context) {
  ...
  for (view, _) in cx.editor.tree.views_mut() {
    for doc_id in &view.jumps.iter().map(|e| e.0).collect::<Vec<_>>() {
    ...
    if line_end_char_index(&text, cursor_line) == cursor_line_start {
      ...
      let pos = match cursor_fallback {
        IndentFallbackPos::LineStart => text
          ...
          .map_or(cursor_line_start, |ws_offset| ws_offset + cursor_line_start),
    ...
    let continue_comment_token =
      if comment_continuation == CommentContinuation::Enabled && config.continue_comments {
        text
          ...
          .and_then(|byte| continued_line_comment_token(doc, &loader, text, curr_line_num, byte))
      } else {
        None
      };
...
pub mod insert {
  ...
  use super::{
    Context, Cow, Deletion, Direction, Document, DynAccess, IndentStyle, KeyCode, KeyEvent, Mode,
    Range, Rope, RopeSlice, RopeSliceExt, Selection, SmallVec, Tendril, Transaction,
    UnicodeWidthChar, append_mode, delete_by_selection_insert_mode, goto_next_tabstop, graphemes,
    indent, insert, insert_mode, move_parent_node_end, movement,
  };
    ...
    let mut transaction = Transaction::change_by_selection(contents, selection, |range| {
      ...
      let continue_comment_token = if config.continue_comments {
        text
          ...
          .and_then(|byte| continued_line_comment_token(doc, &loader, text, current_line, byte))
      } else {
        None
      };
      ...
      let (from, to, local_offs) =
        if let Some(idx) = text.slice(line_start..pos).last_non_whitespace_char() {
          ...
          (
            ...
            pos,
            local_offs.cast_signed() - chars_deleted.cast_signed(),
          )
        } else {
          ...
          (
            line_start,
            line_start,
            new_text.chars().count().cast_signed(),
          )
        };
      ...
      let new_range = if range.cursor(text) > range.anchor {
        ...
        Range::new(
          (range.anchor.cast_signed() + global_offs).cast_unsigned(),
          (range.head.cast_signed() + local_offs + global_offs).cast_unsigned(),
        )
      } else {
        // when inserting, slide the range by local_offs
        Range::new(
          (range.anchor.cast_signed() + local_offs + global_offs).cast_unsigned(),
          (range.head.cast_signed() + local_offs + global_offs).cast_unsigned(),
        )
      };
      ...
      global_offs += new_text.chars().count().cast_signed() - chars_deleted.cast_signed();
...
pub(crate) fn yank_main_selection_to_register(editor: &mut Editor, register: char) {
  ...
  match editor.registers.write(register, vec![selection]) {
    Ok(()) => editor.set_status(format!("yanked primary selection to register {register}")),
    Err(err) => editor.set_error(err.to_string()),
  }
}
...
fn join_selections_impl(cx: &mut Context, select_space: bool) {
  ...
  for selection in doc.selection(view.id) {
    ...
    let mut comment_tokens: Vec<&str> = comment_tokens
      .iter()
  ...
  let line_token: Option<&str> = lang_config
    ...
    .and_then(|tokens| {
      comment::get_comment_token(doc.text().slice(..), tokens, cursor_line)
        .or_else(|| tokens.first().map(std::string::String::as_str))
    });
...
fn goto_ts_object_impl(cx: &mut Context, object: &'static str, direction: Direction) {
  ...
  let motion = move |editor: &mut Editor| {
    ...
    if let Some(syntax) = doc.syntax() {
      ...
      let selection = doc.selection(view.id).clone().transform(|range| {
        let new_range =
          movement::goto_treesitter_object(text, range, object, direction, syntax, &loader, count);
...
fn select_textobject(cx: &mut Context, objtype: textobject::TextObject) {
  ...
  let title = match objtype {
    ...
    textobject::TextObject::Movement => return,
  };
  let help_text = [
    ("w", "Word"),
    ("W", "WORD"),
...
fn shell(cx: &mut compositor::Context, cmd: &str, behavior: &ShellBehavior) {
  ...
  for range in selection.ranges() {
    ...
    let (from, to, deleted_len) = match behavior {
      ...
      ShellBehavior::Insert | ShellBehavior::Ignore => (range.from(), range.from(), 0),
  ...
  ui::prompt(
    ...
    move |cx, input, event| {
      ...
      match Args::parse(input, SHELL_SIGNATURE, true, |token| {
        expansion::expand(cx.editor, token).map_err(std::convert::Into::into)
      }) {
...
fn increment_impl(cx: &mut Context, increment_direction: IncrementDirection) {
  ...
  let mut amount = sign * cx.count().cast_signed();
  ...
  for range in selection {
    ...
    let new_from =
      usize::try_from((i128::try_from(range.from()).unwrap()) + cumulative_length_diff).unwrap();
    let incremented = [increment::integer, increment::date_time]
      .iter()
      .find_map(|incrementor| incrementor(selected_text.as_ref(), i64::try_from(amount).unwrap()));
    ...
    match incremented {
      None => {
        let new_range = Range::new(
          new_from,
          usize::try_from(i128::try_from(range.to()).unwrap() + cumulative_length_diff).unwrap(),
        );
