...
impl Component for Text {
    ...
    fn required_size(&mut self, viewport: (u16, u16)) -> Option<(u16, u16)> {
        if viewport != self.viewport {
            let width = std::cmp::min(u16::try_from(self.contents.width()).unwrap(), viewport.0);
            let height = std::cmp::min(u16::try_from(self.contents.height()).unwrap(), viewport.1);
...
pub fn required_size(text: &tui::text::Text, max_text_width: u16) -> (u16, u16) {
    ...
    for content in &text.lines {
        height += 1;
        let content_width = u16::try_from(content.width()).unwrap();
