...
impl<T: 'static + Send + Sync, D: 'static + Send + Sync> Picker<T, D> {
  pub fn stream(
    ...
    let matcher_columns = u32::try_from(columns.iter().filter(|col| col.filter).count()).unwrap();
            ...
            if metadata.is_dir() {
              ...
              let file_names: Vec<_> = files
                ...
                .filter_map(|(file_path, is_dir)| {
                  let name = file_path
                    .strip_prefix(&path)
                    .map_or_else(|_| file_path.file_name(), |p| Some(p.as_os_str()))?
                    .to_string_lossy();
                  if *is_dir {
      ...
      let doc = match preview.document() {
        ...
        _ => {
          if let Some(dir_content) = preview.dir_content() {
            for (i, (path, is_dir)) in dir_content.iter().take(inner.height as usize).enumerate() {
...
impl<I: 'static + Send + Sync, D: 'static + Send + Sync> Component for Picker<I, D> {
  ...
  fn handle_event(&mut self, event: &Event, ctx: &mut Context) -> EventResult {
    ...
    let close_fn = |picker: &mut Self| {
      ...
      let callback: compositor::Callback = if picker.matcher.snapshot().item_count() > 1_000_000 {
