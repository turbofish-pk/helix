...
impl EditorView {
  ...
  pub fn doc_document_link_highlights(doc: &Document, theme: &Theme) -> Option<OverlayHighlights> {
    let highlight = theme
  ...
  pub fn render_gutter<'d>(
    ...
    for gutter_type in view.gutters() {
      ...
      let gutter_decoration = move |renderer: &mut TextRenderer, pos: LinePos| {
        ...
        if let Some(style) = gutter(pos.doc_line_index, selected, pos.is_first_visual, &mut text) {
              ...
              if let Some(completion) = &mut self.completion {
                  ...
                  && callback.is_some()
                {
                  ...
                  if let Some(cb) = self.clear_completion(cx.editor) {
                    if consumed {
                      cx.on_next_key_callback = Some((cb, OnKeyCallbackKind::Fallback));
                    } else {
